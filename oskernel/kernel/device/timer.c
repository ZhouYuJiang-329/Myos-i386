...
/* 时钟的中断处理函数 */
static void intr_timer_handler(struct interrupt_frame* frame) {
...
/* 初始化PIT8253 */
void timer_init() {
   vga_puts("timer_init start\n");
   /* 设置8253的定时周期,也就是发中断的周期 */
   frequency_set(CONTRER0_PORT, COUNTER0_NO, READ_WRITE_LATCH, COUNTER_MODE, COUNTER0_VALUE);
   interrupt_register_handler(0x20, intr_timer_handler);
   vga_puts("timer_init done\n");
}